defmodule Day3 do
  ...
  def_solution part_1(stream_input) do
    stream_input
    |> to_indexed_bit_frequency
    |> Enum.reduce({"", ""}, fn {_, m}, {gr, er} ->
      if Map.get(m, "0", 0) > Map.get(m, "1", 0) do
        {gr <> "0", er <> "1"}
      else
        {gr <> "1", er <> "0"}
      end
  ...
  def_solution part_2(stream_input) do
    bin_list = Enum.to_list(stream_input)
    bit_frequencies = to_indexed_bit_frequency(bin_list)

    [&Kernel.>/2, &Kernel.<=/2]
    |> Enum.map(fn comparitor -> find_binary(bin_list, bit_frequencies, comparitor) end)
    |> List.to_tuple()
    |> calculate_solution()
  end

  defp find_binary([solution], _, _), do: solution

  defp find_binary(bin_list, [{index, frequencies} | _bit_frequencies], comparitor) do
    zeros = Map.get(frequencies, "0", 0)
    ones = Map.get(frequencies, "1", 0)

    new_bin_list =
      if comparitor.(zeros, ones) do
        Enum.filter(bin_list, fn binary -> String.at(binary, index) == "0" end)
      else
        Enum.filter(bin_list, fn binary -> String.at(binary, index) == "1" end)
      end

    new_bit_frequencies =
      new_bin_list
      |> to_indexed_bit_frequency()
      |> Enum.drop_while(fn {i, _m} -> i <= index end)

    find_binary(new_bin_list, new_bit_frequencies, comparitor)
  end

  defp to_indexed_bit_frequency(stream_input) do
    stream_input
    |> Stream.map(fn line ->
      line
      |> String.codepoints()
      |> Enum.with_index()
      |> Enum.reduce(%{}, fn {b, i}, acc -> Map.put(acc, i, %{b => 1}) end)
    end)
    |> Enum.reduce(
      &Map.merge(&1, &2, fn _k, v1, v2 ->
        Map.merge(v1, v2, fn _k, x1, x2 ->
          x1 + x2
        end)
      end)
    )
  ...
  defp calculate_solution({binary_1, binary_2}) do
    String.to_integer(binary_1, 2) * String.to_integer(binary_2, 2)
  end
