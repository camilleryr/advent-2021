defmodule Day22 do
  ...
  def_solution part_1(stream_input) do
    do_solve(stream_input, &inside_initialization_procedure_region/1)
  end

  def inside_initialization_procedure_region([_, x1..x2, y1..y2, z1..z2]) do
    Enum.all?([x1, x2, y1, y2, z1, z2], fn n -> n in -50..50 end)
  end
  ...
  def do_solve(stream_input, filter_fun \\ & &1) do
    stream_input
    |> parse()
    |> Enum.filter(&filter_fun.(&1))
    |> Enum.reduce([], &build_space/2)
    |> calculate_volume()
  end
  ...
  def volume(cube), do: cube |> Enum.map(&Range.size/1) |> Enum.product()

  def build_space([operation, x_range, y_range, z_range], space) do
    current = [x_range, y_range, z_range]
    updated_space = for cube <- space, sub_cube <- split(cube, current), do: sub_cube

    if operation == :on, do: [current | updated_space], else: updated_space
  end

  def split([c1_x, c1_y, c1_z] = c1, [c2_x, c2_y, c2_z] = c2) do
    if Range.disjoint?(c1_x, c2_x) or Range.disjoint?(c1_y, c2_y) or Range.disjoint?(c1_z, c2_z) do
      [c1]
    else
      do_split(c1, c2)
    end
  ...
  def do_split([cb_start..cb_end | cb_tl], [ch_start..ch_end | ch_tl]) do
    pre = cb_start..(ch_start - 1)//1
    post = (ch_end + 1)..cb_end//1

    do_split(cb_tl, ch_tl)
  ...
  def parse(stream) do
    Enum.map(stream, fn line ->
      ...
      |> Enum.map(fn
        "on" -> :on
        "off" -> :off
        range -> range |> Code.eval_string() |> elem(0)
      end)
    end)
  end
