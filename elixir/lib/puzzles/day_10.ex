defmodule Day10 do
  ...
  def_solution part_1(stream_input) do
    ...
    |> Stream.flat_map(fn line ->
      check_syntax(line, invalid_fun: &List.wrap/1, incomplete_fun: &discard/1)
    end)
    |> Stream.map(fn
      ")" -> 3
      "]" -> 57
  ...
  def_solution part_2(stream_input) do
    ...
    |> Stream.flat_map(fn line ->
      check_syntax(line,
        invalid_fun: &discard/1,
        incomplete_fun: &find_missing(&1, reverse_matches)
      )
    end)
    |> Stream.map(
      &Enum.reduce(&1, 0, fn closer, acc ->
        acc * 5 + points[closer]
      end)
    )
    |> Enum.sort()
  ...
  defp find_missing(matched \\ [], unmatched, reverse_matches)

  defp find_missing(matched, [], _), do: [Enum.reverse(matched)]

  defp find_missing(matched, [opener | unmatched], reverse_matches) do
    closer = reverse_matches[opener]
    find_missing([closer | matched], unmatched, reverse_matches)
  end

  defp check_syntax(head \\ [], tail, opts)

  defp check_syntax(head, [opener | tail], opts) when not is_map_key(@matches, opener) do
    check_syntax([opener | head], tail, opts)
  end

  defp check_syntax([opener | head], [closer | tail], opts) do
    if @matches[closer] == opener do
      check_syntax(head, tail, opts)
    else
      opts[:invalid_fun].(closer)
    end
  end

  defp check_syntax(head, [], opts), do: opts[:incomplete_fun].(head)

  defp discard(_), do: []

  defp parse(stream_input) do
    Stream.map(stream_input, &String.codepoints/1)
  end
