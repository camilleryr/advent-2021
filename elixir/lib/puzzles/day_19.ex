defmodule Day19 do
  ...
  def_solution part_2(stream_input) do
    ...
    |> compile()
    |> elem(1)
    |> get_manhattan_distances()
    |> Enum.max()
  end

  def get_manhattan_distances([]), do: []

  def get_manhattan_distances([head | tail]) do
    tail
    |> Enum.map(&get_manhattan_distance(head, &1))
    |> Enum.concat(get_manhattan_distances(tail))
  end

  @doc ~S"""
  ## Example
    iex> get_manhattan_distance({1105,-1205,1229}, {-92,-2380,-20})
    3621
  """
  def get_manhattan_distance({x1, y1, z1}, {x2, y2, z2}) do
    x_dist = get_manhattan_distance([x1, x2])
    y_dist = get_manhattan_distance([y1, y2])
    z_dist = get_manhattan_distance([z1, z2])

    x_dist + y_dist + z_dist
  end

  def get_manhattan_distance(nums) when is_list(nums) do
    nums |> Enum.sort() |> Enum.reduce(&Kernel.-/2)
  end

  def compile([acc | rest]), do: compile(acc, rest, [{0, 0, 0}])

  def compile(acc, [], offsets), do: {acc, offsets}

  def compile(acc, remaining, offsets) do
    {next_acc, next_rem, new_offset} = do_compile(acc, remaining)
    compile(next_acc, next_rem, [new_offset | offsets])
  end

  def do_compile(acc, prev \\ [], [scanner | remaining]) do
    acc_rel = to_rel_distances(acc)
    scanner_rel = to_rel_distances(scanner)

    acc_rel
    |> Enum.filter(fn {key, _v} -> Map.has_key?(scanner_rel, key) end)
    |> case do
      [] ->
        do_compile(acc, [scanner | prev], remaining)

      overlaps ->
        a_overlap =
          Enum.flat_map(overlaps, fn {_, vs} ->
            Enum.flat_map(vs, fn {a, b} -> [{a, []}, {b, []}] end)
          end)
          |> Map.new()

        b_overlap =
          scanner_rel
          |> Enum.filter(fn {key, _v} -> Map.has_key?(acc_rel, key) end)
          |> Enum.flat_map(fn {_, vs} ->
            Enum.flat_map(vs, fn {a, b} -> [{a, []}, {b, []}] end)
          end)
          |> Map.new()
        ...
        a_overlap
        |> Enum.sort()
        |> Enum.find_value(fn {{x, y, z}, _} ->
          b_overlap
          |> gen_rotations()
          |> Stream.with_index()
          |> Enum.find_value(fn {rotated_b_overlap, index} ->
            Enum.find_value(rotated_b_overlap, fn {{rx, ry, rz}, _} ->
              offset_x = x - rx
              offset_y = y - ry
              offset_z = z - rz

              rotated_b_overlap
              |> Map.new(fn {{xx, yy, zz}, v} ->
                {{xx + offset_x, yy + offset_y, zz + offset_z}, v}
              end)
              |> then(fn overlaps ->
                matched_overlaps = map_size(Map.take(overlaps, Map.keys(a_overlap)))

                if matched_overlaps > 1 do
                  {index, {offset_x, offset_y, offset_z}}
                end
                |> case do
                  nil ->
                    nil
          ...
          |> case do
            {rotation_index, {offset_x, offset_y, offset_z}} ->
              scanner
              |> Map.new(fn k -> {k, []} end)
              |> gen_rotations()
              |> Enum.at(rotation_index)
              |> Map.new(fn {{xx, yy, zz}, v} ->
                {{xx + offset_x, yy + offset_y, zz + offset_z}, v}
              end)
              |> Map.merge(Map.new(acc, fn k -> {k, []} end))
              |> Map.keys()
              |> then(fn next_acc ->
                {next_acc, Enum.reverse(prev) ++ remaining, {offset_x, offset_y, offset_z}}
              end)
        ...
        |> case do
          nil -> do_compile(acc, [scanner | prev], remaining)
          other -> other
        end
    end
  end
  ...
  def parse(stream_input) do
    ...
    # |> Enum.map(&to_rel_distances/1)
  end

  def to_rel_distances(points, acc \\ %{})
  def to_rel_distances([], acc), do: acc

  def to_rel_distances([p | rest], acc) do
    rest
    |> Enum.reduce(acc, fn p2, acc ->
      d = get_relative_distance(p, p2)
      Map.update(acc, d, [{p, p2}], &[{p, p2} | &1])
    end)
    |> then(fn new_acc -> to_rel_distances(rest, new_acc) end)
  end

  def get_relative_distance({x1, y1, z1}, {x2, y2, z2}) do
    x = max(x1, x2) - min(x1, x2)
    y = max(y1, y2) - min(y1, y2)
    z = max(z1, z2) - min(z1, z2)
  ...
  @doc ~S"""
  ## Example
  iex> [og | _rest] = all = test_input_2() |> Advent2021.stream() |> parse() |> Enum.map(&Map.new(&1, fn k -> {k, []} end))
  iex> rotations = gen_rotations(og) |> Enum.to_list()
  iex> Enum.all?(all, & &1 in rotations)
  true
  """
  @ninty_degrees :math.pi() / 180 * 90
  def gen_rotations(map) do
    [
      & &1,
      &rotate_z(&1, @ninty_degrees),
      &rotate_z(&1, @ninty_degrees),
      &rotate_z(&1, @ninty_degrees),
      &rotate_y(&1, @ninty_degrees),
      &rotate_z(&1, @ninty_degrees),
      &rotate_z(&1, @ninty_degrees),
      &rotate_z(&1, @ninty_degrees),
      &rotate_y(&1, @ninty_degrees),
    ...
    |> Stream.scan(map, fn rotator, rotated_map ->
      rotator.(rotated_map)
    end)
  end

  def rotate_z(map, theta) do
    Map.new(map, fn {{x, y, z}, val} ->
      x_prime = x * :math.cos(theta) - y * :math.sin(theta)
      y_prime = x * :math.sin(theta) + y * :math.cos(theta)
      {{round(x_prime), round(y_prime), z}, val}
    end)
  end

  def rotate_y(map, theta) do
    Map.new(map, fn {{x, y, z}, val} ->
      x_prime = x * :math.cos(theta) + z * :math.sin(theta)
      z_prime = z * :math.cos(theta) - x * :math.sin(theta)
      {{round(x_prime), y, round(z_prime)}, val}
    end)
  end
