defmodule Day1 do
  ...
  defp chunk_by_3([a, b, c | rest], acc), do: chunk_by_3([b, c | rest], [a + b + c | acc])
  defp chunk_by_3(_, acc), do: Enum.reverse(acc)
  ...
  defp count_increases(list) do
    Enum.reduce(list, {nil, 0}, fn
      ...
      curr, {prev, acc} ->
        new_acc = if curr > prev, do: acc + 1, else: acc
        {curr, new_acc}
    end)
    |> elem(1)
  end
