defmodule Day6 do
  ...
  ## Example
    ...
    iex> part_1("3,4,3,1,2")
    5934
  """
  def_solution part_1(stream_input) do
    do_solve(stream_input, 80)
  end
  ...
  ## Example
    ...
    iex> part_2("3,4,3,1,2")
    26984457539
  """
  def_solution part_2(stream_input) do
    do_solve(stream_input, 256)
  end

  defp do_solve(stream_input, days) do
    stream_input
    |> pare_to_numbers()
    |> run_iterations(days)
    |> get_solution()
  ...
  defp get_solution(map), do: map |> Map.values() |> Enum.sum()

  defp run_iterations(numbers, 0), do: numbers

  defp run_iterations(numbers, iterations) do
    numbers
    |> run()
  ...
  @doc ~S"""
    iex> run(%{1 => 1, 2 => 1, 3 => 2, 4 => 1})
    %{0 => 1, 1 => 1, 2 => 2, 3 => 1}

    iex> run(%{0 => 1, 1 => 1, 2 => 2, 3 => 1})
    %{0 => 1, 1 => 2, 2 => 1, 6 => 1, 8 => 1}
  """
  def run(numbers) do
    Enum.reduce(numbers, %{}, fn
      {0, num}, acc ->
        acc
        |> Map.put(8, num)
        |> Map.update(6, num, fn existing -> existing + num end)

      {timer, num}, acc ->
        Map.update(acc, timer - 1, num, fn existing -> existing + num end)
    end)
  end

  defp pare_to_numbers(stream_input) do
    stream_input
