defmodule Day17 do
  ...
  ## Example
    iex> hits_target(7, 2, 20..30, -5..-10//-1)
    {:hit, 3}

    iex> hits_target(6, 3, 20..30, -5..-10//-1)
    {:hit, 6}

    iex> hits_target(9, 0, 20..30, -5..-10//-1)
    {:hit, 0}

    iex> hits_target(17, -4, 20..30, -5..-10//-1)
    :long

    iex> hits_target(0, 0, 20..30, -5..-10//-1)
    :short
  """
  def hits_target(x \\ 0, y \\ 0, x_vel, y_vel, x_range, y_range, highest \\ 0) do
    highest = max(y, highest)

    cond do
      Enum.member?(x_range, x) and Enum.member?(y_range, y) ->
        {:hit, highest}

      x > x_range.last ->
        :long

      y < y_range.last or (x < x_range.first and x_vel == 0) ->
        :short

      true ->
        next_x_vel = if x_vel == 0, do: 0, else: x_vel - 1
        hits_target(x + x_vel, y + y_vel, next_x_vel, y_vel - 1, x_range, y_range, highest)
    end
  end
  ...
  def parse(stream_input) do
    ...
    |> Enum.flat_map(fn line ->
      ...
      |> Enum.chunk_every(2)
      |> Enum.map(&Enum.sort_by(&1, fn x -> abs(x) end))
