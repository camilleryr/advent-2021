defmodule Day11 do
  defmodule Control do
    defstruct grid: %{}, flashes: 0, iterations: 0, complete_fun: nil
  end
  ...
  def_solution part_1(stream_input) do
    do_solve(stream_input, fn c -> c.iterations == 100 end, :flashes)
  end
  ...
  def_solution part_2(stream_input) do
    do_solve(stream_input, fn c -> Enum.all?(c.grid, &match?({_, 0}, &1)) end, :iterations)
  end

  defp do_solve(stream_input, complete_fun, key) do
    stream_input
    |> parse()
    |> run(complete_fun)
    |> Map.get(key)
  ...
  def run(grid, complete_fun), do: run(struct(Control, grid: grid, complete_fun: complete_fun))

  def run(control) do
    updated_control =
      control
      |> increment_energey_levels()
      |> reset_flashers()
      |> Map.update!(:iterations, &(&1 + 1))

    if control.complete_fun.(updated_control), do: updated_control, else: run(updated_control)
  end

  def reset_flashers(control) do
    control.grid
    |> Map.filter(fn {_, val} -> val > 9 end)
    |> Map.map(fn _ -> 0 end)
    |> then(fn reset ->
      %{
        control
        | grid: Map.merge(control.grid, reset),
          flashes: control.flashes + map_size(reset)
      }
    end)
  end

  def increment_energey_levels(grid, []), do: grid

  def increment_energey_levels(grid, [to_increment | rest]) do
    case grid[to_increment] do
      val when val in 0..9 ->
        updated_grid = Map.put(grid, to_increment, val + 1)
        updated_rest = if val == 9, do: rest ++ neighbors(to_increment), else: rest
        increment_energey_levels(updated_grid, updated_rest)

      _ ->
        increment_energey_levels(grid, rest)
    end
  end

  def increment_energey_levels(control) do
    %{control | grid: increment_energey_levels(control.grid, Map.keys(control.grid))}
  end
  ...
  """
  def neighbors({og_x, og_y} = og) do
    for x <- (og_x - 1)..(og_x + 1), y <- (og_y - 1)..(og_y + 1), {x, y} != og do
      {x, y}
    end
  end
